...
int Helper::MultipleNumbers(int a, int b)
{
 signed int result; 
  if (a > 0) 
  {  /* si_a is positive */
...
char Helper::IntSymbolToChar(int token)
{
	char symbol = '\0';
	symbol = token + 'a' - 1;
	return symbol;
...
int Helper::CharSymbolToIndex(char token)
{
	int idx = -1;
	if(islower(token)) 
		idx = token - 'a';
...
void Helper::PrintError(char const *s)
{
	printf("\nError in line %d: %s\n", gLineNum , s);
	exit(-1);
} 
...
void AssignmentPolynom(char variable_p,struct Expression *polynom)
{
	...
	int symbols_idx = Helper::CharSymbolToIndex(variable_p);
	...
	for(int p_1 = 0; p_1 < COUNT_MONOM; p_1++)
	{
		...
		for(int p_2 = 0; p_2 < COUNT_MONOM; p_2++)
		{
			...
			//multiply coefficients
				...
				piece_polynom->structure[0][0] = Helper::MultipleNumbers(polynom_2->structure[p_2][0], polynom_1->structure[p_1][0]);
...
{
	int idx = Helper::CharSymbolToIndex(variable_p);
	return &symbols[idx];
}
...
void PrintPolynom(int idx)
{
	...
	for(i = 0; i < COUNT_MONOM; i++)
	{
		...
		if(coeff != 0)
		{
			...
			number = abs(coeff);
			if(number < 0)
				Helper::PrintError("Overflow number");
			if(number != 1)
				printf("%d", number);
			flag_begin_monom = 1;
		}
